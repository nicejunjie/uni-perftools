...
## snap.json  (snapshot collector — RUN-LEVEL, not per-rank)
Counters are read on the whole process tree from outside, so there is exactly
one snapshot per run (no rank key).
```json
